...
# Append an overlay to the colon-separated COMPOSE_FILE list, if not already present.
add_compose_overlay() {
  local overlay="$1" files
  files="$(get_env COMPOSE_FILE)"
  [ -n "$files" ] || files="docker-compose.prod.yml"
  case ":$files:" in
...
# Remove an overlay from the colon-separated COMPOSE_FILE list, if present.
remove_compose_overlay() {
  local overlay="$1" files out part
  files="$(get_env COMPOSE_FILE)"
  [ -n "$files" ] || return 0
  out=""
  local IFS=":"
  for part in $files; do
    [ "$part" = "$overlay" ] && continue
    out="${out:+$out:}$part"
  done
  set_env COMPOSE_FILE "$out"
}

# Generate the GCS overlay (#138): bind-mounts the host service-account key into the app
# container read-only. The GCS settings themselves live in .env; only the mount is templated
# here because its source is a host path chosen during the interview.
write_gcs_overlay() {
  local key_path="$1"
  cat >docker-compose.gcs.yml <<EOF
# Generated by scripts/install.sh — Google Cloud Storage photo backend (#138).
# Mounts your service-account key into the app container read-only; the STAMPORAMA_GCS_*
# and GOOGLE_APPLICATION_CREDENTIALS settings live in .env. Re-run the installer to change.
services:
  app:
    volumes:
      - "${key_path}:/secrets/gcs-key.json:ro"
EOF
}
...
$db_hint" "$(dflt DATABASE_URL "")"
  ...
  # --- Photo storage backend (#138) ---------------------------------------
  # Runs after the DB-mode step so it appends to the COMPOSE_FILE list that step just set
  # (base, or base:network) without clobbering it.
  local storage_mode storage_default
  storage_default="filesystem"
  if [ "$is_reconfigure" -eq 1 ]; then
    case "$(get_env STAMPORAMA_STORAGE_BACKEND)" in gcs) storage_default="gcs" ;; esac
  fi

  ui_menu storage_mode "$storage_default" "Where should uploaded photos be stored?" \
    filesystem "Local disk (the stamporama-data Docker volume)" \
    gcs        "Google Cloud Storage bucket (served via signed URLs)"

  if [ "$storage_mode" = "gcs" ]; then
    local gcs_bucket gcs_key_path gcs_prefix gcs_ttl
    ui_prompt gcs_bucket "GCS bucket name (STAMPORAMA_GCS_BUCKET)" \
      "$(dflt STAMPORAMA_GCS_BUCKET "")"
    [ -n "$gcs_bucket" ] || die "A bucket name is required for GCS storage."

    ui_prompt gcs_key_path "Absolute path on THIS host to the service-account key JSON.
It is mounted read-only into the container and also signs the read URLs." \
      "$(dflt STAMPORAMA_GCS_KEY_HOST_PATH "")"
    [ -n "$gcs_key_path" ] || die "A service-account key path is required for GCS storage."
    [ -f "$gcs_key_path" ] || warn "No file at $gcs_key_path yet — create it before starting, or the container will fail to mount it."

    ui_prompt gcs_prefix "Optional object-key prefix inside the bucket (blank for none)" \
      "$(dflt STAMPORAMA_GCS_KEY_PREFIX "")"
    ui_prompt gcs_ttl "Signed-URL lifetime in seconds" \
      "$(dflt STAMPORAMA_GCS_SIGNED_URL_TTL_SECONDS 300)"
    [ -n "$gcs_ttl" ] || gcs_ttl="300"

    set_env STAMPORAMA_STORAGE_BACKEND "gcs"
    set_env STAMPORAMA_GCS_BUCKET "$gcs_bucket"
    set_env STAMPORAMA_GCS_KEY_PREFIX "$gcs_prefix"
    set_env STAMPORAMA_GCS_SIGNED_URL_TTL_SECONDS "$gcs_ttl"
    set_env GOOGLE_APPLICATION_CREDENTIALS "/secrets/gcs-key.json"
    # Installer bookkeeping only (unused by the app): remembers the host key path so a
    # reconfigure run can pre-fill it and regenerate the mount overlay.
    set_env STAMPORAMA_GCS_KEY_HOST_PATH "$gcs_key_path"

    write_gcs_overlay "$gcs_key_path"
    add_compose_overlay "docker-compose.gcs.yml"
    info "Photos will be stored in gs://${gcs_bucket} (existing filesystem photos keep serving)."
  else
    set_env STAMPORAMA_STORAGE_BACKEND "filesystem"
    remove_compose_overlay "docker-compose.gcs.yml"
...
print_summary() {
  ...
  echo >/dev/tty
  if [ "$(get_env STAMPORAMA_STORAGE_BACKEND)" = "gcs" ]; then
    echo "Uploaded photos are stored in Google Cloud Storage (gs://$(get_env STAMPORAMA_GCS_BUCKET))." >/dev/tty
    echo "Check the backend + health probe in the logs: docker compose logs app | grep '\[storage\]'" >/dev/tty
    echo "Migrate old filesystem photos: docker compose exec app pnpm photos:migrate:gcs --delete-source" >/dev/tty
  else
