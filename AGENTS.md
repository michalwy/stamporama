...
- Assistant **distribution** (#254; ADR-0016) — the instance serves the extension. `extension/pack.mjs` (`pnpm assistant:pack` from the root) builds a signed CRX3 stamped with the app version; CI signs it per release tag with the `ASSISTANT_CRX_KEY` secret and bakes it into `public/assistant/`, which `/assistant/update.xml` advertises to Chrome. Machines force-install it via `ExtensionInstallForcelist`; the update URL lives in the policy entry, not the manifest. The unpacked dev build and the released build are **deliberately two extensions** with two IDs (`extension/identity.mjs`; `manifest.json` carries no `key` — `build.mjs` stamps the dev one, `pack.mjs` the signing key's), so both can live in one browser without colliding and a dev profile can never reach a production collection. Changing either key breaks every machine's policy entry; both IDs are asserted in `extension/crx.test.ts`.
