...
- **An assignment begins with the wake-up drill, unconditionally**: `git fetch origin main`, cut `task/<issue>-<slug>` from `origin/main`, `pnpm install`, then **`pnpm prisma:generate`**. **The fetch and the cut come before you read anything** — this file, `docs/agents/collaboration.md` and the topic files included. A pooled worktree is as old as the pool, so reading a process file out of it before the cut hands you the file as it stood hours ago, honestly, with nothing to say so; after the cut the worktree *is* `origin/main` and every one of them is current. Read with `git show origin/main:<path>` anything you must read before the cut (#1060). The last step looks redundant and is not. A pooled worktree may be hours old when it is assigned, and if `main` has taken a migration since, `pnpm install` reports *Already up to date*, skips the postinstall, and leaves a Prisma client that is **stale rather than missing** — which compiles, and whose tests pass against a schema the branch no longer declares (#862). Regenerating costs seconds; a stale client looks exactly like a sound one. → [`docs/agents/collaboration.md`](docs/agents/collaboration.md)
