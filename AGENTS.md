...
- A **filter never unticks anything, and a bulk action on a list never reaches a row the collector cannot see**: the bar counts and acts on the ticked rows **in view**, hidden ticks survive the filter, and **pruning a selection is never narrowed to the filtered set**. **A picker dialog is the exception** — it submits the whole selection and its button says how many rows are hidden. → `ui-patterns.md`
