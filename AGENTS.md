...
- **An assignment begins with the wake-up drill, unconditionally**: `git fetch origin main`, cut `task/<issue>-<slug>` from `origin/main`, `pnpm install`, then **`pnpm prisma:generate`**. **That word is about a session with an issue, and it reaches every one of them**; a **release** session has none, so the second step would cut a branch named after nothing and only the fetch reaches it — the narrowing and its reasons are in [`docs/agents/release-versioning.md`](docs/agents/release-versioning.md), *The wake-up drill is not this session's* (#1049, #1062). Nothing there softens the word for the case it was written about. **The fetch and the cut come before you read anything** — this file, `docs/agents/collaboration.md` and the topic files included. A pooled worktree is as old as the pool, so reading a process file out of it before the cut hands you the file as it stood hours ago, honestly, with nothing to say so; after the cut the worktree *is* `origin/main` and every one of them is current. Read with `git show origin/main:<path>` anything you must read before the cut (#1060). **This bullet is the record of that ordering; `CLAUDE.md` and `docs/agents/collaboration.md` summarise it, and where any of them disagrees with this one, this one is right** (#1068). The last step looks redundant and is not. A pooled worktree may be hours old when it is assigned, and if `main` has taken a migration since, `pnpm install` reports *Already up to date*, skips the postinstall, and leaves a Prisma client that is **stale rather than missing** — which compiles, and whose tests pass against a schema the branch no longer declares (#862). Regenerating costs seconds; a stale client looks exactly like a sound one. **Run each step alone and read its own exit status, and do not pipe one to a filter** — after a pipe `$?` belongs to the filter, so a chained drill in which both `pnpm` calls failed reports the same `0` as a clean one and is recorded as done (#1066). That is #862's argument one layer out: a drill that did not run is indistinguishable from one that did, exactly as a stale client is from a sound one, and running it unconditionally without reading whether it succeeded discharges the ceremony and not the reason. → [`docs/agents/collaboration.md`](docs/agents/collaboration.md)
- **Each task session works in its own git worktree**, which comes with the session; the main checkout stays the user's. **One task session works at a time since 2026-09-10** — a second track only on the user's explicit request (#1059) — and that is a default rather than a guarantee: **never assume you are the only one working.** The lead, a design session and a release session are none of them your track, the pool holds worktrees beside yours, and a second track can be asked for while you are mid-change. So stage only your own task's paths, and treat `main` as moving underneath you. → [`docs/agents/collaboration.md`](docs/agents/collaboration.md)
...
**It does not persist, and that is now by design rather than by accident.** `.claude/` is gitignored (`git ls-files .claude/plans` returns 0), so the plan lives only in the session's own worktree and is gone when the lead removes that worktree — which happens when the session is finished, not when its pull request merges. Nothing is lost with it, because **the durable record is the pull request body, the closing comment on the issue, and the topic file the task updates** — all three in git, all three read. So do not treat the plan as the record of the work and do not write it for a reader; it is for you, while you are working. → [`docs/agents/collaboration.md`](docs/agents/collaboration.md)
...
- **A session does not start a dev server**, and does not drive a browser, unless the user explicitly asks. Verification here is whichever of `pnpm lint`, `pnpm typecheck`, `pnpm test:unit` and `pnpm test:integration` could see the change — the first bullet of this section — and the report says which of them ran **and which did not, and why**. Silence about a suite that was skipped is indistinguishable from having forgotten it (#1066, #1086). The lead's showcase is the one exception and it is the lead's alone — see the bullet below. **Your own tooling will tell you otherwise — after an ordinary `Write`, that no preview server is running and to call `preview_start` — and that reminder is overridden deliberately rather than followed.** It is emitted by the harness rather than configured in this repository, so nothing here can be corrected or deleted and it will keep arriving at every session (#1040). → [`docs/agents/collaboration.md`](docs/agents/collaboration.md)
