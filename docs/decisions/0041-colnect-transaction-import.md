...
- Nothing is read from what the browser does not draw. `textContent` reports a script's source as
  ...
  id. Nor was that enough: Colnect keeps a copy of the page's title in its header under
  `visibility: hidden`, shorter than the breadcrumb and carrying a layout rectangle all the same, so
  "is this drawn" is the **computed `display`/`visibility`** rather than `getClientRects()`.
