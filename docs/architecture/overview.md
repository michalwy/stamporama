...
- `Offer` — a `Lot` listed on one platform (`Lot` 1:N `Offer`): `platformId` → `Contact` with the `platform` role (`onDelete: Restrict`), `url?`, `price` `Decimal(10,2)` + `currency` (per-offer, platforms price independently), `state` = `active | paused | sold | withdrawn` (+ a derived "to-close" flag). `onDelete: Cascade` from both `Collection` and `Lot`.
