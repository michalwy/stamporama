...
The sales model (see [ADR-0012](../decisions/0012-sales-record-model.md), schema in #162) is the mirror image of purchases and the consumer of `Item.costBasis`. **Schema + the pure P/L allocation engine only so far** (#162/#163) — lot composition, offers, and the sale flow land in #164+. It has two orthogonal axes, symmetric to purchases: the platform-agnostic **package** (`Lot`) and the per-platform **listing** (`Offer`); the `Item` is the thread tying platforms together.
