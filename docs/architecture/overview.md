...
- **Assistant packaging & distribution** (#254; ADR-0016) — the instance is the extension's distribution channel, because Chrome will not install a non-store CRX any other way. `extension/crx.mjs` + `pack.mjs` build a signed CRX3 with no dependencies (deterministic ZIP writer + the protobuf header; verified in `extension/crx.test.ts`), stamping the **app's** version into the archived manifest so an upgrade is what publishes a new extension version. **Two build flavours are two extensions** (`extension/identity.mjs`): `build.mjs` (default, unpacked) stamps in a committed dev-only public key, a ` (dev)` name suffix and amber icons → ID `idmgaeimkafaifpfbjonmjdfbmgffcbh`; `build.mjs --release` leaves the manifest plain and `pack.mjs` stamps in the signing key's public half → ID `afaeadeheelibafbmhobdnkblmbckehn`. `manifest.json` itself carries no `key`. That is what lets a dev build and the policy-installed one coexist in one browser — Chrome refuses two extensions with one ID, and per-extension `chrome.storage.local` keeps a dev profile from ever reaching a production collection. CI's `package-extension` job signs once per `v*` tag (`ASSISTANT_CRX_KEY` secret) and both image builds bake the result into `public/assistant/`; the key never enters an image layer. `GET /assistant/update.xml` (`src/app/assistant/update.xml/route.ts`, unauthenticated — Chrome fetches it cookie-less) renders the Omaha manifest from `public/assistant/crx-metadata.json`, deriving the absolute `codebase` from the request (proxy headers first), and 404s on source/dev builds that carry no CRX. `manifest.json` intentionally has **no** `update_url`: the per-machine policy entry (`ExtensionInstallForcelist` = `<id>;https://<instance>/assistant/update.xml`) supplies it, so one CRX serves every instance origin.
