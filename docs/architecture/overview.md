...
- Statuses/kinds are `String` columns (no native Postgres enums), enforced by the domain layer. The other guards — one active offer per (item × platform), quantity decrement — are derived/domain-level.
