...
- `Sale` — the transaction header: single `platformId` → `Contact` (`onDelete: Restrict`), `soldAt` (date, FX frozen here), one `currency`, `fxRateToBase?` `Decimal(65,30)`, and three shared amounts — `buyerHandling?` (+), `shippingCost?` (−, my cost), `commission?` (−, manual) — all `Decimal(10,2)`. Scoped to `Collection` (`onDelete: Cascade`).
- `SaleLine` — the unit lot or sub-lot that sold: `offerId?` → `Offer` (`onDelete: SetNull`, so a historical sale survives offer cleanup), `lotId` → `Lot` (`onDelete: Restrict`), `price` `Decimal(10,2)` in the sale currency. `SaleLineItem` records the exact copies that left; its **`@@unique([itemId])` is the DB-level no-double-sale guard**, and `Item` FK is `onDelete: Restrict`. No sold flag is added to `Item` — sold state is read through this join.
