...
- Every release tag also publishes the Assistant extension: CI packs a store ZIP stamped with the release version and submits it to the unlisted Chrome Web Store listing (#288, ADR-0017). The job needs the `CWS_*` secrets/variables; a green job means *submitted for review*, not live, so a release note should not promise the extension is already updated.
