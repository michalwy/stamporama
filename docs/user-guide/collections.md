...
- **Area tree** (left) — filter to an area and its sub-areas. Each area carries a **count** of how many rows selecting it would show, counted against everything else on the screen — the search box, the year, the other filters — but not against the area filter itself, so the number always says what clicking it would give you. With **include sub-areas** on, a parent's count covers its whole subtree; switch it off and each area counts only what is filed on it directly. With no year selected, an area's count is exactly what the years under it add up to. Areas with nothing in them show **0** rather than an empty space. Nodes with children can be collapsed or expanded, and the whole column can be hidden/shown with the **◂** / **▸** toggle in its header; both are remembered between visits. Clicking the area you already have selected does nothing — to widen the list again, use **All areas** at the top.
- **Year filter** (middle) — the years present in the current results, listed oldest to newest, each with a count of how many items fall in that year. Click a year to narrow the list to it, and **All years** to clear — clicking the year you already have selected does nothing, so a stray second click cannot silently widen the list back to everything. Items with no year appear under a **No year** entry. On the issue list the year comes from the issue's year; on the stamp and copies lists it comes from each stamp's own issue year. The counts update as you change the area filter, search, or the other filters. Use the **◂** / **▸** toggle in the panel header to hide or show this column — the choice is remembered between visits.
...
The area tree's header carries a **＋** button: it opens the same **Add area** dialog the Areas screen uses, so a country or a period you have just realised is missing can be added without leaving the list. Whatever area is selected is pre-filled as the new area's **parent**, so with *Poland* selected the **＋** starts you on a sub-area of Poland; with **All areas** selected it starts a top-level area. Everything else in the dialog behaves exactly as it does on the Areas screen — the new area inherits the numbering and price sources of the parent you picked, and you can change the parent in the dialog itself. The new area appears in the tree straight away, and your current area selection is left alone: a brand-new area holds nothing, so jumping the list onto it would only empty the screen you were working in. The same **＋** is on the [Want list](wants.md) rail, and on the area tree inside the pickers — the **Browse…** stamp picker, **Add set** on an offer, **Add copies** on a trade and **Attach existing copies…** on a purchase lot — so a country you meet halfway through picking can be added there. The Add area dialog opens on top of the picker, **Esc** closes one of them at a time, and when it closes the picker is exactly as you left it: the same ticks, the same search and filters, the same scroll position. The **bulk listing workspace** and the **lot builder** do not have it — there the area tree narrows what you are working on rather than describing your collection.
...
All filtering and sorting happens on the server, and the active area, year, search, and sort are kept in the page URL so a view can be bookmarked or shared. Whatever sort you pick (name, year, issue date, …), items that share the same sort value are then ordered by their **primary catalog number**, so a run of same-year issues still reads in catalog order rather than an arbitrary one. Catalog numbers read the way the catalogue arranges them: numerically (1, 2, 10 — not 1, 10, 2), a number before its lettered variants (200, 200a, 200b), and each family written with a letter prefix as its own block after the basic numbering — the plain numbers first, then Bl 1, Bl 2, Bl 10, then D1, then P1, P2, P15. A number with no digits in it at all, such as a bare Roman numeral, has no place on that scale and sorts last. The **area** and **year** selections are also remembered per collection and shared across these three lists: pick an area and year on one list and they carry over when you switch to another, and they are restored when you come back after visiting an unrelated page. The **catalog** chosen in the catalog-number filter is likewise remembered per collection and shared between the Issues and Stamps lists, so your preferred catalog stays selected across reloads. Other filters (search, catalog number, sort, and the copies-only filters) stay per list. When you add an issue while a year is selected, the new issue's year is pre-filled to match.
...
- a stamp row inside the issue tree — arrow (Open stamp page), pencil (Edit), ruler (Add variant range…), copy (Add copy). **Add child stamp** is in the **⋮** menu: when a catalogue splits a stamp it usually splits it into a whole lettered run, so the range is the one you reach for while working through an issue and the one-at-a-time version is the exception
...
Everything on both sections **inherits down the tree**, and everything shown as a placeholder or in italics is inherited rather than set here. What is inherited always follows the **Parent area** currently picked in the dialog: change the parent from Poland to Germany and the placeholders switch to Germany's values straight away, and pick **None (top-level)** and they empty. Clearing a field hands it back to the parent — it never stores a blank. The area rows on the list show the same picture: the prefix in force, the leading vendor and the other vendors as chips, and the valuing volume, each in *italics* when it comes from an ancestor. So a leaf that declares nothing still tells you what it is using and where it came from, without opening the dialog.
...
A stamp only moves **among its own siblings**: a root stamp among the issue's other root stamps, a variant among the variants of the same parent. Dragging a stamp carries its variants with it. Moving a stamp *out* of its parent is a different thing — see [Filing a stamp under a different one](#filing-a-stamp-under-a-different-one) below.
...
Only where the stamp sits changes. It keeps its catalog numbers, its name and its subtype, and every copy, want, offer and checklist entry that names it is untouched. Its own sub-stamps come with it, and it lands at the end of its new level, where you can drag it into place with **Reorder**.

Two things it will not offer you: the stamp itself, and anything already hanging below it — a stamp cannot be filed under its own variant. A stamp with no subtype yet gets the collection's default one when it becomes a variant, since that is what any child stamp carries; one that already has a subtype keeps it.

The parent must be a stamp of the **same issue**. To file a stamp under one in a *different* issue, first **Move to another issue…**, then reassign it there — two steps, each of which you can see the result of.

Everything the tree decides follows immediately: which stamp counts as a variant of which, what a [checklist](#checklists) counts as held, and which catalog value rolls up to the parent. Both the old parent and the new one are right the next time you look at them.

To nest a *new* stamp **under an existing one**, use **Add child stamp** in that stamp's **⋮** menu — on the Issues list or in the Stamps card of the [issue's own page](detail-pages.md#the-issue-page) — or **Add variant** on the stamp's own [detail page](detail-pages.md#the-stamp-page), which opens the same dialog. The dialog starts from the parent: its catalog numbers are copied in as a starting point, and the **Year** is pre-filled from the **parent stamp's own year** rather than the issue's — a variant or reprint is usually dated from the node it hangs under. (A root-level stamp still starts from the issue's year.) After you save, the parent node **opens by itself** so the new child is visible instead of hiding behind a collapsed arrow.

#### Adding a range of variants

When the catalogue splits one stamp into a lettered run — `240a` through `240f` — adding them one at a time is six passes through the same dialog for six stamps that differ in one character. **Add variant range…** in the stamp's **⋮** menu (on the Issues list or on the [issue's own page](detail-pages.md#the-issue-page)), or **Add range** on the [stamp's own page](detail-pages.md#the-stamp-page), creates the whole run in one save.
...
Every card also has a **🔗 source** button at the right of its footer, beside the title. Use it to record **where the picture came from** — the auction, Colnect page or forum thread you took a screenshot from, or a book citation typed in as text. It is most useful on **reference photos**: a picture of a genuine stamp added as an extra, or a picture on a forgery's own stamp, is worth little once you can no longer say where you found it. Click the button to type or change the source (Enter or clicking away finishes; clear the text to remove it); hover it to read the source back. The button is tinted while a photo has one. Anything up to 2,000 characters is accepted, and nothing checks that it is a link. The source is saved with the rest of the dialog, and nothing else in the app reads it.
...
Each stamp, issue and **copy** row's **⋮** actions menu has a **Show valuation** action that opens the **Valuation** dialog — for a single stamp, or, on an issue row, for the whole issue's required stamps. From a row on the **Copies** list it opens for the stamp that copy is of, read-only: close it and you are back on the list where you were. A **piece carrying several stamps** is a copy of none of them, so its row opens a window of its own instead — the value you recorded on the piece, and the sum of its stamps as a suggestion — described in [What a piece carrying several stamps is worth](inventory.md#what-a-piece-carrying-several-stamps-is-worth); the rest of this section is about the stamp window. An **auction lot's composition lines** offer it too, so a stamp's record can be checked while a bid ceiling is being decided — see [Auction tracking](auctions.md#checking-what-a-stamp-is-worth-from-the-line). The data is loaded on demand when you open the dialog. It has five kinds of section, each in the same collapsible box and each on the same grid — **conditions as rows, certificate statuses as columns**:
...
- **What I paid** (open by default, third) — what the copies of this stamp you **still hold** cost you, in the collection currency. Each cell is the **average** cost of the copies at that condition and certificate status; hover it for the range, the counts, and when the most recent of them was bought. Copies are counted three ways under the figure, and the difference matters:
  ...
  A cell can show a **—** with counts under it: that means you hold copies at that condition but none of them has a settled cost yet. Sold copies and ones you no longer hold are not counted — this answers *what did what I have cost me*. Nor are copies from an [opening balance](purchases.md#opening-balances): their cost is an opening value, what you said the stamps were worth rather than a price anybody paid, and one of them would bend the average. Copies are counted for **that stamp exactly**: an unknown-variant umbrella does not borrow its variants' copies, the same rule the copies-held badge follows. This section appears for a stamp, not for an issue's checklist.
