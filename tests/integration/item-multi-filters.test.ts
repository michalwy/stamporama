...
// The Copies list's multi-value filters (#425, #427). The condition and delivery-state axes are
// plain `in`s and are covered here mostly so an OR stays an OR; what is genuinely worth pinning down
// is **format**, where `"single"` is the *null* format (ADR-0020) and therefore cannot ride in an
// `in` list — ticking it beside a real format has to be two branches, and ticking it alone has to
// stay the null test the single-select already was.
...
describe("multi-value copy filters", () => {
  ...
  before(async () => {
    ...
    await createItem(userId, collectionId, { stampId, conditionId: mhId, formatId: blockId });
