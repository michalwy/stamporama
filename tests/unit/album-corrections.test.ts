...
describe("a corrected box still comes out of the drawer", () => {
  ...
  // chosen number: it is the height of the shortest strip in stock the piece fits into, because
  // hawid is sold as strips of a fixed height that are cut across. A correction applied to the
  // finished box would draw one at a height no strip has — the page-disagrees-with-the-desk failure
  // #765 exists to prevent.
  const stock = [
    { heightMm: 24, stockLengthMm: 210 },
    { heightMm: 29, stockLengthMm: 210 },
    { heightMm: 41, stockLengthMm: 210 },
  ];
  ...
  it("moves the height in strip steps, not continuously", () => {
    // 20 + 4 = 24 needs the 24 mm strip. Two more millimetres on the piece is 26, which no longer
    // fits it — so the box jumps to 29, and the drag that asked for 2 got 5. That is the correct
    // answer and the screen has to be able to say so.
    assert.equal(box(null).heightMm, 24);
    assert.equal(box({ widthDeltaMm: 0, heightDeltaMm: 2 }).heightMm, 29);
    // And a correction the current strip absorbs changes nothing at all.
    assert.equal(box({ widthDeltaMm: 0, heightDeltaMm: -1 }).heightMm, 24);
  });
  ...
  it("makes a box nothing can supply a pocket, which is an answer rather than an error", () => {
    ...
    const raised = box({ widthDeltaMm: 0, heightDeltaMm: 40 });
    assert.equal(raised.strip, null);
    assert.equal(raised.heightMm, 64);
