...
describe("the collector's copy of the partner link (#681)", () => {
  it("seals a minted token and reads it back", () => {
    ...
    assert.deepEqual(readShareAddress(sealed), { readable: true, token: TOKEN });
  });
  ...
  it("mints without a key rather than refusing to share", () => {
    ...
    assert.equal(sealShareToken(TOKEN), null);
  });

  it("says a link minted before #681 needs regenerating", () => {
    assert.deepEqual(readShareAddress(null), { readable: false, reason: "legacy" });
  });
  ...
  it("says a missing key is the thing to fix, not the link", () => {
    ...
    delete process.env[SECRET_KEY_ENV];
    assert.deepEqual(readShareAddress(null), { readable: false, reason: "unconfigured" });
    assert.deepEqual(readShareAddress("v1.a.b.c"), {
      readable: false,
      reason: "unconfigured",
    });
  });
  ...
  it("says a key that changed under a sealed link is why it will not open", () => {
    ...
    assert.deepEqual(readShareAddress(sealed), { readable: false, reason: "unreadable" });
  });

  it("reads a corrupt value as unreadable rather than throwing", () => {
    assert.deepEqual(readShareAddress("not-a-sealed-value"), {
      readable: false,
      reason: "unreadable",
    });
  });
