...
export function AlbumPageCanvas({
  ...
  /** Which box a stamp-order drag started on, so a drop knows what it is moving. */
  const carrying = useRef<{ blockId: string; stampId: string } | null>(null);
  /** Which block a heading drag started on, for the same reason one level up. */
  const carryingBlock = useRef<string | null>(null);
        ...
        return (
          <g
            ...
            onPointerDown={(e) => {
              ...
              if (!readOnly) carryingBlock.current = block.id;
            }}
            onPointerUp={() => {
              const held = carryingBlock.current;
              carryingBlock.current = null;
              if (!held || !block || held === block.id) return;
              onReorderBlocks(held, block.id);
            }}
          >
            ...
            <rect
              ...
              height={Math.max(heading.heightMm, heading.face.lineHeightMm)}
              fill="transparent"
            />
          ...
          <g key={`b${i}`}>
            ...
            <rect
              ...
              onPointerDown={(e) => {
                ...
                if (!readOnly) carrying.current = { blockId, stampId: box.stampId };
              }}
              onPointerUp={() => {
                const held = carrying.current;
                carrying.current = null;
                if (!held || held.blockId !== blockId || held.stampId === box.stampId) return;
